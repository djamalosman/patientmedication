               ...
               <div class="sidebar-menu">
                   <ul class="menu">
                       ...
                       @foreach (main_module(1) as $main_module)
                           @if ($main_module->module->xpadding == 1)
                               <li class="sidebar-item  has-sub">
                                   <a href="#" class='sidebar-link'>
                                       <i class="bi bi-collection-fill"></i>
                                       <span>{{ $main_module->module->description }}</span>
                                   </a>
                                   <ul class="submenu">
                                       @foreach (main_menu(1) as $main_menu)
                                           @if ($main_menu->module->xpadding == 2)
                                               @if ($main_menu->module->upline == $main_module->module->name)
                                                   <li class="sidebar-item  has-sub">
                                                       <a href="#" class='sidebar-link'>
                                                           <i class="bi bi-stack"></i>
                                                           <span>{{ $main_menu->module->description }}</span>
                                                       </a>
                                                       <ul class="submenu ">
                                                           @foreach (main_submenu(1) as $main_submenu)
                                                               @if ($main_submenu->module->xpadding == 3)
                                                                   @if ($main_submenu->module->upline == $main_menu->module->name)
                                                                       <li class="submenu-item ">
                                                                           @if ($main_submenu->module->xurl == null)
                                                                               <a
                                                                                   href="{{ $main_submenu->module->xurl }}">{{ $main_submenu->module->description }}
                                                                               @else
                                                                                   <a
                                                                                       href="{{ route($main_submenu->module->xurl) }}">{{ $main_submenu->module->description }}
                                                                           @endif
